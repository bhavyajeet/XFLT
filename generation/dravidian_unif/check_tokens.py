...
langlist  = ['te','kn','ml','ta','hi','mr','or','pa']
...
for lang in langlist:
    ...
    for line in read_file:
        ...
        tokenzier_args = {'text': uni_sentence, 'truncation': True, 'pad_to_max_length': False, 
                                    'max_length': 1024, 'return_attention_mask': True}
        tokenized_data = tokenizer.encode_plus(**tokenzier_args)
        postnum = len(tokenized_data['input_ids'])
        postok[number].append(postnum)
        postover.append(postnum)
        fin_number += (postnum - prenum)/prenum
        tot_len += 1 

    print (fin_number/tot_len)
