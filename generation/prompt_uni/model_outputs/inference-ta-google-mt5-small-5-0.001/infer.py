...
pred_file = open('test-predicted-epoch-4.txt')
jsonl_file = open('lang.jsonl')
...
for i in jsonl_file.readlines() :
    ...
    if (totpred > totfacts):
        if 'country_of_citizenship' in pred and 'country_of_citizenship' not in ref:
            totpred -= 1
        if 'occupation' in pred and 'occupation' not in ref:
            totpred -= 1
        else :
            print ('-'*30)
