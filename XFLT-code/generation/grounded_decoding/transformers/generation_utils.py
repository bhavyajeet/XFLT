...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        #print (tokenizer.batch_decode(encoder_input_ids, skip_special_tokens=True))
        src_string =  tokenizer.batch_decode(encoder_input_ids, skip_special_tokens=True)
        #print(src_string, type(src_string))

        sim_func = model_kwargs.pop('sim_func')
        fact_str_emb = model_kwargs.pop('fact_embs')
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        sim_func = model_kwargs.pop("sim_func")
        fact_str_emb = model_kwargs.pop('fact_embs')
        #print (sim_func('lol','generate punjabi : <h> bindu a. bambah <r> educated_at <t> university of chicago <qr> point_in_time <qt> 1983',F.cosine_similarity))
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        if max_length is not None:
            warnings.warn(
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            strs_inp = tokenizer.batch_decode(input_ids, skip_special_tokens=True)
            #print(strs_inp)
            outputs = self(
                **model_inputs,
                return_dict=True,
            ...
            next_token_scores, next_tokens = torch.topk(
                next_token_scores, 4 * num_beams, dim=1, largest=True, sorted=True
            )
            ...
            #print(src_string)
            sim_toks = []
            #print (next_tokens.shape[1])
            for i in range(next_tokens.shape[1]):
                toks = next_tokens[:, i:i+1]
                strs = tokenizer.batch_decode(toks, skip_special_tokens=True)
                int_sim_toks = []
                """
                bigtoksimlist  = sim_func(strs,fact_str_emb,F.cosine_similarity)
                int_sim_toks = torch.vstack(bigtoksimlist)
                """
                for tokens, inp_str in zip(strs, fact_str_emb):
                    sim_tok = sim_func(tokens, inp_str, F.cosine_similarity).mean()
                    int_sim_toks.append(sim_tok)
                int_sim_toks = torch.vstack(int_sim_toks)
                sim_toks.append(int_sim_toks)
            sim_toks = torch.hstack(sim_toks).to(next_tokens.device)
            #print(sim_toks.shape)
            tau = 0.5
            ##print(sim_toks)
            returned_ground_scores = F.log_softmax(sim_toks/tau, dim=1)
            #print(returned_ground_scores)
            ...
            alpha = 0.5
            ##print ("before us ",next_token_scores)
            ##print ("ours ", returned_ground_scores)
            next_token_scores = next_token_scores + alpha * returned_ground_scores
            #print ("after us ",next_token_scores)
